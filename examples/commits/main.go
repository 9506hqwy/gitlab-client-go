...
func main() {
	...
	resp, err := c.GetApiV4ProjectsIdRepositoryCommitsWithResponse(context.TODO(), "2", &params, bearerAuth)
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, commit := range *resp.JSON200 {
		fmt.Printf("Id: %s, Date: %s, Message: %s\n", *commit.Id, *commit.AuthoredDate, *commit.Message)
	}
}
