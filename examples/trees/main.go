...
func main() {
	...
	resp, err := c.GetApiV4ProjectsIdRepositoryTreeWithResponse(context.TODO(), "2", &params, bearerAuth)
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, tree := range *resp.JSON200 {
		fmt.Printf("Name: %s, Path: %s, Id: %s\n", *tree.Name, *tree.Path, *tree.Id)
	}
}
